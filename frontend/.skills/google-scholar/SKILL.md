...
| Property | Value |
|----------|-------|
| Endpoint | `GET https://app-e8yvrjq9s9hd-api-Xa6JZq2055oa.gateway.appmedo.com/search` |
...
```json
{
  ...
  "pagination": {
    "current": 1,
    "next": "https://app-e8yvrjq9s9hd-api-Xa6JZq2055oa.gateway.appmedo.com/search?start=10"
  }
}
```
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-e8yvrjq9s9hd-api-Xa6JZq2055oa.gateway.appmedo.com/search?${queryParams.toString()}`,
    {
      method: "GET",
      headers: {
