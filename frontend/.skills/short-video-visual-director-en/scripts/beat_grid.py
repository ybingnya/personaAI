...
def parser():
    p=argparse.ArgumentParser(description="Generate a beat and bar timing grid")
...
def make_grid(bpm,fps,beats_per_bar,bars):
    if bpm<=0 or fps<=0 or beats_per_bar<1 or bars<1: raise ValueError("All parameters must be positive")
    spb=60/bpm; rows=[]
...
def main():
    ...
    if a.csv_path:
        with open(a.csv_path,"w",newline="",encoding="utf-8") as f:
            w=csv.DictWriter(f,fieldnames=["beat","bar","beat_in_bar","seconds","frame"]); w.writeheader(); w.writerows(data["grid"])
    print(json.dumps(data,indent=2) if a.json or not a.csv_path else a.csv_path)
    return 0
